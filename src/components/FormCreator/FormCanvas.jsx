...
import { createForm } from "../../utils/api/form";
...
export default function SimpleFormBuilder() {
  ...
  const updateOptions = (id, newOptions) => setFields((prev) => prev.map((f) => (f.id === id ? { ...f, options: newOptions } : f)));
  ...
  const handleSaveForm = async () => {
    if (!formName.trim())
      return setAlert({ show: true, title: "Missing name", description: "Enter the form name first", color: "danger" });
    if (fields.length === 0)
      return setAlert({ show: true, title: "No fields", description: "Please add at least one field before saving.", color: "danger" });

    try {
      setIsSaving(true);
      await createForm(baseUrl, { name: formName, uid, fields });
  ...
  const next = () => {
    if ((step === 1 && !canGoNextFrom1) || (step === 2 && !canGoNextFrom2)) return;
  ...
  const renderField = (f) => {
    ...
    switch (f.type) {
      ...
      case "rating":
        return <StarRatingField {...common} options={f.options} />;
      case "radio":
        return <RadioField {...common} options={f.options || []} onOptionsChange={(opts) => updateOptions(f.id, opts)} />;
      case "email":
  ...
  const container = `max-w-4xl mx-auto p-6 min-h-screen transition-colors duration-300 ${
    darkMode ? "text-gray-100" : "text-gray-900"
  }`;
  const card = `rounded-2xl transition-colors duration-300 ${darkMode ? "bg-transparent" : "bg-transparent"}`;

  const StepDot = ({ title, active, done }) => (
    <div className="flex items-center gap-3">
      <motion.div
        layout
        initial={{ scale: 1 }}
        animate={done ? { scale: 1.18, backgroundColor: "#10B981" } : active ? { scale: 1.08 } : { scale: 1 }}
  ...
  const connectorProgress = step === 1 ? 0 : step === 2 ? 0.5 : 1;

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.32 }} className={container}>
      <div className="mb-4">
        <AnimatePresence>
          {alert.show && (
      ...
      <div className={`${card} p-3 mb-6`}>
        <div className="flex items-center gap-4">
          <StepDot title="Name" active={step === 1} done={step > 1} />
          ...
          <StepDot title="Form" active={step === 2} done={step > 2} />
          <div className="relative flex-1 h-[4px] rounded-full mx-3 bg-gray-200/60 dark:bg-white/8 overflow-hidden">
            <motion.div
              style={{ position: "absolute", left: 0, top: 0, bottom: 0 }}
          ...
          <StepDot title="Create" active={step === 3} done={false} />
        </div>
      </div>
      ...
      <AnimatePresence mode="wait" initial={false}>
        {step === 1 && (
          <motion.section key="step-1" variants={stepVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.22 }} className={`${card} p-6`}>
              ...
              <div className="flex gap-2">
                ...
                <motion.button
                  ...
                  className={`px-4 py-2 rounded font-bold flex items-center gap-2 ${
                    canGoNextFrom1
                      ? darkMode
                        ? "bg-[var(--lightblue)] text-black hover:bg-[var(--blue)]"
