...
import { patchForm,deleteForm } from "../utils/api/form";
...
export default function FeedbackCard({ form, setEmbedForm, setShareForm, onDelete, onUpdate }) {
  ...
  const handleOpenLink = () => setShareForm?.(form);
  const handleOpenCode = () => setEmbedForm?.({ ...form, showCodeOnly: true });
  ...
  const handleDelete = async () => {
    ...
    if (!id) return showAlert("danger", "Delete Failed", "Form ID not found");
    try {
      setIsDeleting(true);
      await deleteForm(baseUrl, id);
      onDelete?.(id);
  ...
  const handlePauseToggle = async () => {
    const id = form.id || form.customId;
    if (!id) return showAlert("danger", "Action Failed", "Form ID not found");
    const newPaused = !form.paused;
    try {
      setIsPausing(true);
      const resp = await patchForm(baseUrl, id, { paused: newPaused });
      const updated = resp?.form ?? { ...form, paused: newPaused };
      onUpdate?.(updated);
  ...
  const submitLimit = async () => {
    const id = form.id || form.customId;
    if (!id) return setLimitError("Form ID missing");
    const trimmed = String(tempLimit).trim();
    const MIN = 0;
    const MAX = 100000;
    let payload;
    if (trimmed === "") payload = { feedbackLimit: null };
    else {
      const n = Number(trimmed);
      if (!Number.isFinite(n) || !Number.isInteger(n)) return setLimitError("Please enter a whole number");
      if (n < MIN || n > MAX) return setLimitError(`Value must be between ${MIN} and ${MAX}`);
      payload = { feedbackLimit: Math.floor(n) };
    }
    try {
      setIsSettingLimit(true);
      const resp = await patchForm(baseUrl, id, payload);
      const updated = resp?.form ?? { ...form, feedbackLimit: payload.feedbackLimit };
      onUpdate?.(updated);
    ...
    <>
      <motion.article
        ...
        className={`relative flex flex-col justify-between p-5 rounded-2xl border ${
          darkMode
            ? "bg-[var(--darker)] border-gray-700 text-gray-100 shadow-sm"
      ...
      >
              ...
              {menuOpen && (
                <motion.div
                  ...
                  className={`absolute right-0 mt-2 w-48 rounded-lg overflow-hidden z-20 ${
                    darkMode
                      ? "bg-[var(--darker)] border border-gray-700 shadow-lg"
                ...
                >
                  <button
                    ...
                    className="w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-gray-50 hover:text-red-600 dark:hover:bg-white/6"
                  >
                    <FiTrash2 /> Delete
                  </button>
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex items-center gap-3 ${
                      isPausing
                        ...
                        : "hover:bg-gray-50 hover:text-gray-700 dark:hover:bg-white/6"
                    }`}
                  >
                    <FiPause /> {form.paused ? "Unpause" : "Pause"}
                  </button>
                  <button
                    onClick={openSetLimitModal}
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            <div className={`inline-block text-[12px] px-3 py-1 rounded-full ${darkMode ? "bg-white/6 text-gray-200" : "bg-gray-200 text-gray-700"}`}>{created}</div>
          </div>
          {form.description && (
        ...
        {deleteModalOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className={`relative z-10 max-w-md w-full mx-4 rounded-xl p-6 shadow-2xl ${
                darkMode ? "bg-[#0b0b0b] text-gray-100" : "bg-white text-gray-900"
              }`}
            >
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className={`px-3 py-2 rounded text-sm border ${darkMode ? "border-gray-700" : "border-gray-200"}`}
                  disabled={isDeleting}
                >
                  Cancel
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded text-sm font-semibold ${
                    isDeleting ? "opacity-60 cursor-not-allowed" : "bg-red-600 text-white hover:bg-red-700"
                  }`}
                  disabled={isDeleting}
                >
      ...
      <AnimatePresence>
        {limitModalOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex items-center justify-center"
          >
            <div className="absolute inset-0 bg-black/40" onClick={() => !isSettingLimit && setLimitModalOpen(false)} />
            <motion.div
              initial={{ y: 8, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 8, opacity: 0 }}
              transition={{ duration: 0.18 }}
              className={`relative z-10 max-w-md w-full mx-4 rounded-xl p-5 shadow-2xl ${
                darkMode ? "bg-[#0b0b0b] text-gray-100" : "bg-white text-gray-900"
              }`}
            >
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded border outline-none mb-2 ${
                  darkMode
                    ? "bg-[#111214] border-gray-700 text-gray-100"
